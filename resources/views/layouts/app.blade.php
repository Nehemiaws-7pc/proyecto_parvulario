...
</div><div class="d-flex flex-column flex-lg-row align-items-lg-center gap-3 text-white ms-lg-4 py-3 py-lg-0"><div class="text-lg-end small"><a class="profile-nav-link" href="{{ route('profile.edit') }}"><x-line-icon name="family" class="nav-icon" /> <span class="fw-semibold">Mi perfil</span></a><div class="opacity-75">{{ $currentUser->nombre }} · {{ $currentUser->role->etiqueta }}</div></div><form method="POST" action="{{ route('logout') }}" data-confirm="Tu sesión se cerrará en este dispositivo." data-confirm-title="¿Cerrar sesión?">@csrf<button type="submit" class="btn btn-outline-light btn-sm">Cerrar sesión</button></form></div></div>
</div></nav>
@endauth
@yield('content')
